...
/**
 * Copy Right Information: TaskScheduler.java JDK version: jdk1.8.0_11
 * 
...
public class Router {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			/*
			 * we numbered the routers stored in the hashtable by a random
			 * sequence but with an group of consecutive numbers starting from
			 * 0.(notice: this matches every kind of situations except when more
			 * than one routers have the same router name. The correspondence
			 * between router names and numbers are 1-1 but router names are not
			 * in alphabetical order. This will leads to different results from
			 * those which uses other orders of storing data when SHP schemes
			 * are used.)
			 */
			for (String key : keys) {
			...
			for (int i = 0; i < num_of_routers; i++) {
				for (int j = 0; j < num_of_routers; j++) {
					hops[i][j] = Integer.MAX_VALUE/10;
					delay[i][j] = Integer.MAX_VALUE/10;
					load[i][j] = 0;
					firm_load[i][j] = Integer.MAX_VALUE/10;
					occupation[i][j] = 0;
				}
			}
			/*
			...
			/*
			 * scan workload into a new class connection then connect them with
			 * a ArrayList then sort them against starting time.
			 */
		...
		if (ROUTING_SCHEME.equals("SHP")) {
			for (int start = 0; start < num_of_routers; start++) {
				...
				DijkstraAlgorithm dij = new DijkstraAlgorithm(tempHops, start);
		...
		} else if (ROUTING_SCHEME.equals("SDP")) { // SDP or LLP(first time)
			for (int start = 0; start < num_of_routers; start++) {
				...
				DijkstraAlgorithm dij = new DijkstraAlgorithm(tempDelay, start);
				for (int end = 0; end < num_of_routers; end++) {
					if (start == end) {
						continue;
					}
	...
	private static void process(ArrayList<Connection> connection,
		...
		try {
			for (Connection con : connection) {
				...
				List<Integer> route = null; // route which stores the current most
										// optimized route
				...
				if (scheme.equals("SHP") || scheme.equals("SDP")) {
					...
					float max = 0;
					for (int i = 0, j = 1; i < pathLength - 1 && j < pathLength; i++, j++) {
						if (occupation[mark[i]][mark[j]] > max) {
							max = occupation[mark[i]][mark[j]];
						}
						if (max == 1) {
							lost = 1;
							break;
	...
	private void calculate() {
		...
		try {
			...
			for (int count = 1; count < dimention; count++) {
				...
				visited[bridge] = 1;
				/*
				 * if distance between start and end stored in graph now is
				 * bigger than the plus of distances from the start to the
				 * bridge and from the bridge to the end, then replace the old
				 * one with the new cost which is less than before.
				 */
				for (int end = 0; end < dimention; end++) {
					if (visited[end] == 0
